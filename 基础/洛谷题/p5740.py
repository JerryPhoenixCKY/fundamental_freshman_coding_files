# P5740 【深基7.例9】最厉害的学生
## 题目描述
# 现有 $N$ 名同学参加了期末考试，并且获得了每名同学的信息：姓名（不超过 $8$ 个字符的仅有英文小写字母的字符串）、语文、数学、英语成绩（均为不超过 $150$ 的自然数）。
...
# n = int(input())
# students = [(a,b,c,d) for a,b,c,d in  [input().split()] for _ in range(n)]
# grades=[sum(map(int,student[1:])) for student in students]
# max_index=grades.index(max(grades))
# output = ' '.join(map(str, students[max_index]))
# print(output)
...
print(' '.join(map(str, best_student)))
