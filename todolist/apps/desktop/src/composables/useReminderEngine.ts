...
function showBrowserNotification(task: TaskItem) {
...
export function useReminderEngine(tasks: Ref<TaskItem[]>) {
  const notices = ref<ReminderNotice[]>([]);
  ...
  function upsertTaskInStore(task: TaskItem) {
