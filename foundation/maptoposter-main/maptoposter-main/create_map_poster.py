...
MANUAL_COORDS_FILE = CACHE_DIR / "manual_coordinates.json"
...
# Local coordinate fallback for common cities. This avoids external geocoding
# dependency in restricted network environments.
LOCAL_COORDINATE_FALLBACKS: dict[tuple[str, str], tuple[float, float]] = {
...
def normalize_location_text(value: str) -> str:
    """Normalize location input by trimming and collapsing spaces."""
    return " ".join(value.strip().lower().split())


def normalize_location(city: str, country: str) -> tuple[str, str]:
    """Normalize and alias-map city/country names for matching and caching."""
    city_norm = normalize_location_text(city)
    country_norm = normalize_location_text(country)
    city_norm = CITY_ALIASES.get(city_norm, city_norm)
    country_norm = COUNTRY_ALIASES.get(country_norm, country_norm)
    return city_norm, country_norm


def load_manual_coordinates() -> dict[tuple[str, str], tuple[float, float]]:
    """
    Load optional manual coordinate mappings from cache/manual_coordinates.json.
    ...
    try:
        with MANUAL_COORDS_FILE.open("r", encoding=FILE_ENCODING) as f:
            raw = json.load(f)
    except (OSError, json.JSONDecodeError) as e:
        print(f"⚠ Could not read manual coordinate file '{MANUAL_COORDS_FILE}': {e}")
        return {}
    ...
    for key, value in raw.items():
        ...
        city_raw, country_raw = key.split("|", 1)
        city_norm, country_norm = normalize_location(city_raw, country_raw)
        try:
            lat = float(value[0])
            lon = float(value[1])
        ...
        parsed[(city_norm, country_norm)] = (lat, lon)

    return parsed


def resolve_geocode_result(location):
    """Resolve coroutine geocoder result when needed."""
...
def get_coordinates(city, country):
    """
    Fetch coordinates for a given city and country.

    Resolution order:
    1) Cache
    2) Manual local file (cache/manual_coordinates.json)
    3) Built-in fallback city table
    4) Online geocoding (Nominatim, with retries)
    """
    city_norm, country_norm = normalize_location(city, country)
    coords = f"coords_{city_norm}_{country_norm}"
    ...
    # Local manual mapping takes priority and avoids network dependencies.
    manual_map = load_manual_coordinates()
    local_coords = manual_map.get((city_norm, country_norm))
    if local_coords is not None:
        print(f"✓ Using manual coordinates for {city}, {country}")
        try:
            cache_set(coords, local_coords)
        except CacheError as e:
            print(e)
        return local_coords

    # Built-in fallback for frequently used cities.
    builtin_coords = LOCAL_COORDINATE_FALLBACKS.get((city_norm, country_norm))
    if builtin_coords is not None:
        print(f"✓ Using built-in fallback coordinates for {city}, {country}")
        try:
            cache_set(coords, builtin_coords)
        except CacheError as e:
            print(e)
    ...
    query_candidates = [
        f"{city}, {country}",
        f"{city_norm}, {country_norm}",
        city,
    ]

    # Preserve order while removing duplicates
    unique_queries = list(dict.fromkeys(q.strip() for q in query_candidates if q.strip()))
    ...
    for query in unique_queries:
        for attempt in range(1, 4):
            ...
            try:
                ...
                location = resolve_geocode_result(location)
            except Exception as e:
                last_error = e
                print(
                    f"⚠ Geocoding attempt failed ({attempt}/3) with query '{query}': {e}"
                )
                continue
            ...
            print(f"✓ Coordinates: {resolved[0]}, {resolved[1]}")
            try:
                cache_set(coords, resolved)
            except CacheError as e:
                print(e)
            return resolved
    ...
    raise ValueError(
        ...
        "No local coordinates found and online providers were unavailable. "
        "Pass --latitude and --longitude, or add an entry in "
        f"{MANUAL_COORDS_FILE} using key city|country, e.g. guangzhou|china. "
        f"Last geocoding error: {last_error}"
    )
